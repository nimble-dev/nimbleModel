...
declFunBase_nClass <- nClass(
  ...
  Rpublic = list(
    ...
    calc_op = function(instr, fn) {
      ...
      if(instr$type == 11) return(calc_2_generic(instr, fn))
    ...
    calc_2_x_y_ord =
        function(instr, fn) {
            ...
            index_types1 <- instr$types[1] 
            index_types2 <- instr$types[2] 
            for(i in 1:instr$lens[1]) {
    ...
    calc_3_allseq = 
        function(instr, fn) {
            ...
            for(i in iStart1:(iStart1 + instr$lens[1] - 1)) {
                ...
                for(j in iStart2:(iStart2 + instr$lens[2] - 1)) {
                    idx[2] <- j
                    for(j in iStart3:(iStart3 + instr$lens[3] - 1)) {
                        idx[3] <- k
                        logProb <- logProb + self[[fn]](idx)
                    }
                }
    ...
    simulate = function(instr) {
        ...
        if(instr$type == 11) return(sim_2_generic(instr))
