...
modelDeclClass <- R6Class(
  ...
  public = list(
    ...
    processDecl = function(nimFunNames, constants = list(), envir) {
      ...
      if(!identical(declRule$fullRange$extractIndexRange()$numElements,
                    declRule$originalIndexingRule$apply('y')$extractIndexRange()$numElements))
        stop("found duplicated node definitions in declaring `", declRule$expr, "`.")
        ...
        if (length(RHSreplaced) > 1) { # It actually has argument(s).
          ...
          if (truncated) { # Check for user-provided constant bounds inconsistent with distribution range.
            ...
            if (is.numeric(boundExprs$lower_) &&
              ...
              messageIfVerbose(
                "   [Warning] Lower bound is greater than or equal to upper bound in `",
            ...
            if (is.numeric(boundExprs$lower_) &&
              ...
              boundExprs$lower_ < distRange$lower) {
              messageIfVerbose(
                "   [Warning] Lower bound is less than or equal to distribution lower bound in `",
            ...
            if (is.numeric(boundExprs$upper_) &&
              ...
              messageIfVerbose(
                "   [Warning] Upper bound is greater than or equal to distribution upper bound in `",
                safeDeparse(calculateCode),
        ...
        if (any(dynamicIndices)) {
          ...
          for (iIndex in which(dynamicIndices)) {
            ...
            dynamicIndexInfo[[length(dynamicIndexInfo) + 1]] <<-
              list(
                indexCode = stripIndexWrapping(symbolicParent[[2 + iIndex]]),
                lower = lower,
                upper = upper
              )
...
checkForIndexedIntervals <- function(indexExpr, context) {
  if (length(indexExpr) > 1 && indexExpr[[1]] == ":" && any(all.vars(indexExpr) %in% context$indexVarNames)) {
    return(TRUE)
  } else {
    return(FALSE)
  }
