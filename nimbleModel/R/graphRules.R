    ...
    apply = function(fromVarRange, removeDuplicates = TRUE) {
      ...
      if (is.character(fromVarRange)) {
        if (fromVarName == fromVarRange && numFromIndexSlots) {
          ...
          fromVarRange <- varRangeClass$new(fromVarRange)
        } # string providing the varRange
      }
...
makeSeparableIndexSets <- function(toExpr, fromExpr, context) {
  ...
  toBoolIndexVarList <- lapply(
    toIndexExprs,
    makeBoolIndexVarList
  )
  fromBoolIndexVarList <- lapply(
    fromIndexExprs,
    makeBoolIndexVarList
  )
  ...
  while (length(remainingIndexVarNames)) {
    ...
    while (!done) {
      ...
      allAdditionalIndexVarNames <- setdiff(
        unique(c(
          toAdditionalIndexVars,
    ...
    remainingIndexVarNames <- setdiff(
      remainingIndexVarNames,
      currentIndexVarNames
    )
  }
  ...
  return(list(
    toIndexSlotToSet = toIndexSlotToSet,
...
applyGraphRule <- function(fromVarRange, rule, varName = NULL, removeDuplicates = TRUE) {
  ...
  if (length(constantRulesIdx)) {
    constantIndexRanges <- sapply(
      indexRules[constantRulesIdx],
