...
samplerConfClass <- R6Class(
    ...
    public = list(
        ...
        initialize = function(name, samplerFunction, target, control, model) {
            ...
            if(name == 'crossLevel')
                control <<- c(control,
                              list(dependent_nodes = getNodes(model, getDependencies(model$modelDef, target, self = FALSE), stochOnly = TRUE)))  ## special case for printing dependents of crossLevel sampler (only)
        },
...
mcmcConfClass <- R6Class(
    ...
    public = list(
        ...
        initialize = function(model, nodes, control = list(), useConjugacy = TRUE, ...) {
            ...
            nodeRanges <- c(getNodes(model, nodes = nodes, stochOnly = TRUE, includeData = FALSE, topOnly = TRUE),
            ...
            for(i in seq_along(nodeRanges))
                addDefaultSampler(nodeRange = nodeRanges[[i]],
        ...
        addDefaultSampler = function(nodeRange, control = list(), useConjugacy = TRUE, ...) {
            ...
            if(model$isBinary(nodeRange)) { addSampler(target = nodeRange, type = 'binary', control = controlDefaultsArg);     invisible(NULL) }
            
            ## for categorical nodes, assign a 'categorical' sampler
            if(dist == 'dcat') { addSampler(target = nodeRange, type = 'categorical', control = controlDefaultsArg);     invisible(NULL) }
