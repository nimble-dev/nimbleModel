  ...
  if (.sort) {
    ...
    if (nodesAsChars) {
      if (returnScalarComponents) {
        ...
        result <- lapply(result, \(x) x$toNodeChars())
      }
      result <- unlist(result)
    }
  }
...
intersect_nodes <- function(nodeSet1, nodeSet2) {
  if (!length(nodeSet1) || !length(nodeSet2)) {
    return(list())
  }
  ...
  newNodeSet1 <- list()
  length(newNodeSet1) <- length(nodeSet1)
...
getConditionallyIndependentSets <- function(model, nodes, givenNodes,
  ...
  if (!givenProvided) {
    givenNodes <- c(
      model$getNodes(topOnly = TRUE, stochOnly = TRUE, nodesAsChars = FALSE),
  ...
  if (!nodesProvided) {
    nodes <- model$getNodes(
      latentOnly = TRUE, stochOnly = TRUE,
  ...
  if (nodesAsChars) {
    if (returnScalarComponents) {
      ...
      sets <- lapply(sets, \(set) flatten(lapply(set, \(x) x$toNodeChars())))
    }
  }
  return(sets)
}
...
setupMargNodes <- function(model, paramNodes, randomEffectsNodes, calcNodes,
  ...
  if ((!reProvided) || check) {
    ...
    if (!allowDiscreteLatent && length(latentNodes)) {
      ...
      if (any(latentDiscrete)) {
        if ((!reProvided) && check) {
          messageIfVerbose(
            "  [Note] In trying to determine default `randomEffectsNodes`, there are some nodes\n",
    ...
    if (!calcProvided) {
      if (reProvided) {
        ...
        tempDataNodesDefault <- model$getNodes(dataOnly = TRUE, nodesAsChars = FALSE)
      }
      if (paramsHandled) {
      ...
      if (paramsHandled) { # This means reProvided OR paramsProvided. Including parents allows checking
        ...
        reNodesDefault <- intersect_nodes(
          reNodesDefault,
  ...
  if (reProvided && check) {
    ...
    if (length(reCheck)) {
      ...
      messageIfVerbose(
        "  [Warning] There are some random effects (latent states) in the model that look like\n",
  ...
  if (calcProvided && check) {
    ...
    if (length(calcCheck)) {
      ...
      messageIfVerbose(
        "  [Warning] There are some model nodes that look like they should be\n",
  ...
  if (calcOtherProvided && check) {
    ...
    if (length(calcOtherCheck)) {
      ...
      if (lengthMissingStochNodes) {
        ...
        messageIfVerbose(
          "  [Warning] There are some model nodes (stochastic) that look like they should be\n",
  ...
  if (length(calcNodesOther)) {
    if (!paramDetermDepsCalculated) {
      ...
      paramDetermDeps <- model$getNodes(model$getDependencies(paramNodes, nodesAsChars = FALSE),
        determOnly = TRUE,
  ...
  if (length(randomEffectsNodes)) {
    if (isFALSE(split)) {
      ...
      if (isTRUE(split)) {
        ...
        givenNodes <- setdiff_nodes(
          c(paramNodes, calcNodes),
          c(
            randomEffectsNodes,
...
splitLatents <- function(model, paramNodes, latentNodes, calcNodes, calcNodesOther, control = list()) {
  ...
  if (any(discreteStochNodes)) {
    stop(
      "splitLatents: found discrete non-data stochastic nodes in processing nodes for quadrature-based posterior approximation: ",
  ...
  margNodes <- setupMargNodes(
    model = model, paramNodes = paramNodes, randomEffectsNodes = latentNodes,
  ...
  if (missing(paramNodes) && missing(latentNodes)) {
    ...
    margNodes <- setupMargNodes(
      model = model, paramNodes = paramNodes,
