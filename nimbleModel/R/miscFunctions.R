# These are commented out to avoid loading the nimble namespace
# # This needs to be sourced after `nimbleFunction` is defined, so can't be done in `distributions_inputList.R`.
calc_dmnormAltParams <- function() stop("Don't call this")
...
calc_dwishAltParams <- function() stop("Don't call this")
