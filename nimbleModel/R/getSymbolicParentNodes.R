...
getSymbolicParentNodes <- function(code,
                                   ...
                                   envir) {
  if (addDistNames) {
    nimbleFunctionNames <- c(
      nimbleFunctionNames,
      getAllDistributionsInfo("namesExprList")
    )
  }
  ans <- getSymbolicParentNodesRecurse(
    code,
    constNames,
    indexNames,
    nimbleFunctionNames,
    ...
    if (indexingBracket) {
      ...
      if (variable$replaceable) {
        ...
        if (all(contentsReplaceable)) {
          ...
          return(list(
            code = c(contentsCode, list(dynamicIndexParent)),
            replaceable = FALSE,
            hasIndex = any(contentsHasIndex)
          ))
        }
...
expandDynamicIndex <- function(expr) {
  if (length(expr) > 1 && expr[[1]] == ".DYN_INDEXED") {
    return(substitute(1:M, list(M = .Machine$integer.max)))
  } else {
    return(expr)
  }
}
