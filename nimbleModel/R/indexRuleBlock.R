  ...
  public = list(
    ...
    apply = function(indexRange, collapse = TRUE) {
      ...
      switch(class(indexRange)[1],
        ...
        indexRangeSequenceClass = indexRuleBlock_applyToSequence(
...
indexRuleBlock_setup <- function(toIndexExprList,
  ...
  if (length(wh)) {
    stop(
      "constant `", paste(unique(varsInExpr[wh]), collapse = ","),
      "` not found in `constants`."
    )
  }
  fromRange <-
    c(
      eval(indexRangeExpr[[2]], envir = constants),
