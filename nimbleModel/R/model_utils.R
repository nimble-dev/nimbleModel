...
makeVecName <- function(name) {
  if (length(name) == 0) {
    return(name)
  } # This was causing a problem when we have blank modelValues (i.e. MCMCconf if we have no thinning monitors)
  paste0(name, "_Vec")
}
  ...
  if (is.call(code)) {
    if (safeDeparse(code[[1]], warn = TRUE) == "[") {
      if (safeDeparse(code[[2]], warn = TRUE) %in% nodesToExtract) {
        ...
        if (is.null(allSizeAndDimList[[safeDeparse(code[[2]], warn = TRUE)]])) {
          allSizeAndDimList[[safeDeparse(code[[2]], warn = TRUE)]][[1]] <- thisCodeExprList
        } else {
          allSizeAndDimList[[safeDeparse(code[[2]], warn = TRUE)]][[length(allSizeAndDimList[[safeDeparse(code[[2]], warn = TRUE)]]) + 1]] <- thisCodeExprList
        }
...
parseTreeSubstitute <- function(pt, pattern, replacement) {
  ...
  } # for now turn logicals into numerics until we propagate logicals to C++
  for (i in seq_along(pt)) {
    pt[[i]] <- parseTreeSubstitute(pt[[i]], pattern, replacement)
  }
  return(pt)
}
