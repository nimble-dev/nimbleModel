...
distributionsInputList <- list(
  ...
  dbeta = list(
    BUGSdist = "dbeta(shape1, shape2, mean, sd)",
  ...
  ddexp = list(
    BUGSdist = "ddexp(location, rate, scale, var)",
    Rdist = c(
  ...
  dgamma = list(
    BUGSdist = "dgamma(shape, rate, scale, mean, sd)",
    Rdist = c(
  ...
  dinvgamma = list(
    BUGSdist = "dinvgamma(shape, scale, rate)",
    Rdist = c("dinvgamma(shape, rate = 1/scale)"),
    altParams = c(
      "scale = 1/rate",
      "mean = 1 / (rate * (max(shape,1)-1))",
      "sd = 1 / (rate * (max(shape,1)-1) * sqrt(max(shape,2)-2))"
    ), # max ensures Inf moment when appropriate
    range = c(0, Inf),
    pqAvail = TRUE
  ),
  ...
  dsqrtinvgamma = list(
    BUGSdist = "dsqrtinvgamma(shape, scale, rate)",
  ...
  dlogis = list(
    BUGSdist = "dlogis(location, rate, scale)",
    Rdist = "dlogis(location, scale = 1/rate)",
    altParams = "rate = 1/scale",
    pqAvail = TRUE
  ),
  dnorm = list(
    BUGSdist = "dnorm(mean, tau, sd, var)",
    Rdist = c(
      "dnorm(mean, sd = 1/sqrt(tau))",
  ...
  dt = list(
    BUGSdist = "dt(mu, tau, df, sigma, sigma2)",
    Rdist = c(
      "dt_nonstandard(df, mu, sigma = 1/sqrt(tau))",
  ...
  dunif = list(
    BUGSdist = "dunif(min, max, mean, sd)",
  ...
  dweib = list(
    BUGSdist = "dweib(shape, lambda, scale, rate)",
    Rdist = c(
      "dweibull(shape, scale = lambda^(-1/shape))",
  ...
  dcar_normal = list(
    BUGSdist = "dcar_normal(adj, weights, num, tau, c, zero_mean)",
    Rdist = c(
  ...
  dmnorm = list(
    BUGSdist = "dmnorm(mean, prec, cov, cholesky, prec_param)",
    Rdist = c(
      "dmnorm_chol(mean, cholesky = chol(prec), prec_param = 1)",
      "dmnorm_chol(mean, cholesky = chol(cov), prec_param = 0)",
      "dmnorm_chol(mean, cholesky, prec_param)"
    ),
    altParams = c(
      "prec = calc_dmnormAltParams(cholesky, prec_param, 1)",
  ...
  dmvt = list(
    BUGSdist = "dmvt(mu, prec, df, scale, cholesky, prec_param)",
    Rdist = c(
      "dmvt_chol(mu, cholesky = chol(prec), df = df, prec_param = 1)",
  ...
  dlkj_corr_cholesky = list(
    BUGSdist = "dlkj_corr_cholesky(eta, p)",
    Rdist = "dlkj_corr_cholesky(eta, p)",
    types = c("value = double(2)")
  ),
  dwish = list(
    BUGSdist = "dwish(R, df, S, cholesky, scale_param)",
    Rdist = c(
      "dwish_chol(cholesky = chol(R), df, scale_param = 0)",
      "dwish_chol(cholesky = chol(S), df, scale_param = 1)",
      "dwish_chol(cholesky, df, scale_param)"
    ),
    altParams = c(
      "R = calc_dwishAltParams(cholesky, scale_param, 0)",
  ...
  dinvwish = list(
    BUGSdist = "dinvwish(S, df, R, cholesky, scale_param)",
    Rdist = c(
      "dinvwish_chol(cholesky = chol(S), df, scale_param = 1)",
